  ...
  for (var i = 0; i < _milestones.length; i++) {
    ...
    if (isLast || correct < _milestones[i + 1].threshold) {
      return (
        ...
        nextTarget:
            isLast ? _milestones[i].threshold : _milestones[i + 1].threshold,
  ...
  List<Widget> _buildClueProgressCards() {
    ...
    for (final meta in _clueTypeMetas) {
      ...
      cards.add(
        Padding(
          padding: const EdgeInsets.only(bottom: 10),
          child: _ClueProgressCard(meta: meta, correctCount: correct),
        ),
      );
    }
    return cards;
...
class _ClueProgressCard extends StatelessWidget {
  const _ClueProgressCard({required this.meta, required this.correctCount});
  ...
  Widget build(BuildContext context) {
    ...
    final progress =
        range > 0 ? (progressInRange / range).clamp(0.0, 1.0) : 1.0;
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Icon(meta.icon, color: FlitColors.gold, size: 20),
          ),
          const SizedBox(width: 14),
          // Progress info
          Expanded(
